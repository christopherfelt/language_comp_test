...
export const handler = async (event) => {
    ...
    async function getWiktionaryData(word){
        ...
        return await axios.get('https://en.wiktionary.org/w/api.php?action=parse&format=json&uselang=user&page='+word).then(resp => {
    ...
    try{
        response = {
            status:200,
            body: await getWiktionaryData(event.queryStringParameters.word)
